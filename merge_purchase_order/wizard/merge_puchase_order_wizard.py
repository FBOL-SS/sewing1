...
class MergePurchaseOrder(models.TransientModel):
    ...
    merge_type = \
        fields.Selection([
            ('new_cancel',
                'Crear un nuevo pedido y cancelar todos los pedidos de compra seleccionados'),
            ('new_delete',
             'Crear un nuevo pedido y eliminar todos los pedidos de compra seleccionados'),
            ('merge_cancel',
             'Unir en una orden seleccionada existente y cancelar otras'),
            ('merge_delete',
                'Unir en una orden seleccionada existente y eliminar otras')],
            default='new_cancel')
    ...
    def merge_orders(self):
        ...
        if any(order.state != 'draft' for order in purchase_orders):
            raise UserError(
                _('Seleccione los pedidos de compra que esten en estado de Presupuesto '
                  'Para realizar la Unión.'))
        partner = purchase_orders[0].partner_id.id
        if any(order.partner_id.id != partner for order in purchase_orders):
            raise UserError(
                _('Seleccione Órdenes de compra cuyos proveedores sean iguales  '
                    ' Para realizar la Unión.'))
