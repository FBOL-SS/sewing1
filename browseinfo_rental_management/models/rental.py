...
class RentalOrder(models.Model):
	...
	invoice_ids = fields.Many2many("account.invoice", string='Invoices', readonly=True,compute="_get_invoiced")
	...
	def create(self, vals):
		...
		if vals.get('rental_bill_freq_type') == 'days':
			calc = vals.get('rental_bill_freq',0.00)/30
			if calc > vals.get('rental_initial',0.00):
				raise Warning('Invoice cycle period should not be grater then total rental period')
		if vals.get('rental_bill_freq_type') == 'months':
			if vals.get('rental_bill_freq', 0.00) > vals.get('rental_initial',0.00):
				raise Warning('Invoice cycle period should not be grater then total rental period')
	...
	def _create_picking(self):
		...
		for rental in self:
			pick_type = self.env['stock.picking.type'].search([
				...
				('warehouse_id', '=', rental.warehouse_id.id)]).id
	...
	def _get_invoiced(self):
		...
		"""
		for rental in self:
			invoice = self.env['account.invoice'].search([('rental_id','=',rental.id)])
			rental.update({
...
class SaleRentalOrderLine(models.Model):
	...
	invoice_lines = fields.Many2many('account.invoice.line', string='Invoice Lines', copy=False)
...
class RantalOrderLine(models.Model):
	...
	product_uom_qty = fields.Float(string='Quantity', digits=dp.get_precision('Product Unit of Measure'), default=1.0)
	price_unit = fields.Float('Rent', required=True, digits=dp.get_precision('Product Price'), default=0.0)
	lot_id = fields.Many2one('stock.production.lot', string='Serial Number', change_default=True)
	invoice_lines = fields.Many2many('account.invoice.line', string='Invoice Lines', copy=False)
	tax_id = fields.Many2many('account.tax', string='Taxes', domain=[('type_tax_use','!=','none'), '|', ('active', '=', False), ('active', '=', True)])
	price_subtotal = fields.Float(compute='_compute_amount', string='Subtotal', readonly=True, store=True)
